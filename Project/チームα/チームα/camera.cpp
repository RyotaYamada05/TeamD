...
#define PLAYE_ROT_VERTICAL_FRONT	(D3DXToRadian(180.0f))	//プレイヤーの縦軸左
#define WIN_START_ROT_VERTICAL		(D3DXToRadian(0.0f))	//勝利時縦軸回転の開始角
#define WIN_START_ROT_HORIZONTAL	(D3DXToRadian(40.0f))	//勝利時横軸回転の開始角
#define WIN_START_DISTANCE			(1500.0f)				//勝利時の開始距離
#define WIN_END_ROT_VERTICAL		(D3DXToRadian(360.0f))	//勝利時縦軸回転の終了角
...
void CCamera::EndUpdate(D3DXVECTOR3 PlayerPos[], D3DXVECTOR3 PlayerRot[], int nWinPlayer)
{
	...
	if (nWinPlayer == m_nCameraNum)	//勝利カメラ処理
	{
		...
		if (m_fφ <= 0)
		{
			...
			fDifference = PlayerRot[m_nCameraNum].y + (WIN_END_ROT_VERTICAL - WIN_START_ROT_VERTICAL);				//プレイヤーの角度プラス開始終了角度の差
